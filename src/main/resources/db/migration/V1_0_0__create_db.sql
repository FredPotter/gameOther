...
create table message (is_deleted boolean, id bigint not null, message_date timestamp(6) not null, receiver_id bigint, sender_id bigint, message_subject varchar(255) not null, text varchar(255) not null, primary key (id));
create table message_to_support (is_deleted boolean, id bigint not null, message_date timestamp(6) not null, sender_id bigint, message_subject varchar(255) not null, text varchar(255) not null, primary key (id));
create table offer (category_id bigint, creation_date timestamp(6) not null, game_id bigint, id bigint not null, price_per_lot bigint not null, quantity bigint not null, quantity_goods_in_lot bigint not null, seller_id bigint, vip_status_id bigint unique, description varchar(255) not null, name varchar(255) not null, obtain_method varchar(255) not null, primary key (id));
create table review (moderation_check_status smallint not null check (moderation_check_status between 0 and 1), rating smallint not null, buyer_id bigint, dispatch_time timestamp(6) not null, id bigint not null, seller_id bigint, game_name varchar(255) not null, text varchar(255), primary key (id));
...
create table user_blocking (expiration_date timestamp(6) not null, id bigint not null, user_id bigint, reason varchar(255), primary key (id));
create table users (account_status smallint not null check (account_status between 0 and 1), code integer, balance bigint, country_id bigint, id bigint not null, last_login_date timestamp(6), registration_date timestamp(6) not null, email varchar(255), login varchar(255) not null, password varchar(255) not null, path_to_avatar varchar(255), user_status varchar(255) not null check (user_status in ('ADMIN','USER')), username varchar(255) not null, primary key (id));
create table vip_status (expiration_date_vip_status timestamp(6) not null, id bigint not null, primary key (id));
